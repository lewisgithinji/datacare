...
CREATE TABLE public.chatbot_recommendations (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  conversation_id UUID NOT NULL REFERENCES public.chatbot_conversations(id) ON DELETE CASCADE,
...
-- FAQ responses
CREATE TABLE public.chatbot_faq_responses (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
...
ALTER TABLE public.chatbot_recommendations ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.chatbot_analytics ENABLE ROW LEVEL SECURITY;
ALTER TABLE public.chatbot_faq_responses ENABLE ROW LEVEL SECURITY;
...
ON public.chatbot_recommendations
FOR ALL
USING (true)
WITH CHECK (true);
...
ON public.chatbot_faq_responses
FOR SELECT
USING (active = true);
...
CREATE OR REPLACE FUNCTION public.calculate_chatbot_lead_score(
  org_type TEXT,
  company_size TEXT,
  urgency TEXT,
  budget TEXT
...
INSERT INTO public.chatbot_faq_responses (question_keywords, response, resource_url, category, priority) VALUES
...
-- Success message
SELECT 'Chatbot tables renamed successfully! Now you can run the WhatsApp platform migration.' AS message;
